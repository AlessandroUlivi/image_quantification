...
from single_channel_multi_quantifications import quantify_single_channel
...
class sample_quantifier():
    ...
    def quantify_sample(self, sample_input_folder, roi_maintain=None, roi_exclude=None, roi_3D_maintain=False, roi_3D_exclude=False, roi_position_axis=None):
        ...
        for input_f in list_of_input_files:
            ...
            if self.roi_structure != None:
                ...
                if self.roi_structure in input_f:
                    ...
                    print(input_f, " will be used as a ROI")
        ...
        #Use quantify_single_channel if only one channel is present
        if multi_channel_array.shape[0]==1:
            #Pass the channel_array with the channel dimension in position 0, if an iteration axis is provided
            if self.analysis_axis!=None:
                channels_quantifications = quantify_single_channel(channels_array=multi_channel_array,
            ...
            #Pass the channel_array without the channel dimension in position 0, if an iteration axis is not provided
            else:
                channels_quantifications = quantify_single_channel(channels_array=multi_channel_array[0,...],
                                                                roi_mask_array=roi_channel,
                                                                analysis_axis=quantification_axis,
                                                                no_quantification_valu_e=self.no_quantification_valu_e,
                                                                channels_binarization_thresholds=self.channels_binarization_thresholds,
        ...
        #Use quantify_channels if multiple-channels are present
        else:
            #Quantify channels
